...
The goal of this notebook is to demonstrate how to estimate treatment effects of a dichotomous (binary) treatment on change from a continuous baseline measure (see for example [@tennant2021]) under four different scenarios:
...
# Generate follow-up scores with additive effect
additive_effect <- 10  # improvement of +10 to time1 for treatment group
time1 <- baseline + group*additive_effect + rnorm(n, mean = 0, sd = 3)
```

To generate the RCT outcomes and all the potential outcomes we use the following code (hidden) to get:
...
For publication ready tables, we can use gtsummary. Here is the cannonical Table 1 showing an overview of the dataset:
...
When we have all potential outcomes for the populations, this estimand is identical to the difference in outcome @eq-att-outcome, because the baselines cancel out across the two means below. Because of the following equality
