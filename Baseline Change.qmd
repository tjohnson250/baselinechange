...
datatable(dataMod1, options = list(pageLength = 10))
```
...
Since we have the potential outcomes for all patients we can calculate the Average Treatment Effect of the treatment on `time1` (@eq-att-outcome) as the difference between the mean follow-up outcome between the treated and control groups:
...
datatable(dataMod2, options = list(pageLength = 10))
```
...
Recall that the true causal effect is `{r} additive_effect` and the estimate from this data based on potential outcomes is `{r} po_estimate`. The estimate from the RCT is higher than both. Let's use regression to estimate the difference in means for `time1` between the treatment and control groups and give us a p-value and confidence interval:
...
Here, there are no changes over the marginal effect estimate.
...
Here we see that the analysis of change scores using either the marginal effect of `group` on `change1` or ANCOVA recovers the correct estimate. When analyzing follow-up scores, the marginal effect of `group` on `time1` is biased and has a wide confidence interval, but ANCOVA eliminates the bias and produces a confidence interval equivalent to the the change score analyses.
...
Since baseline is not a confounder and there are no other confounders in the causal DAG for our data generating model, by causal DAG theory baseline does not need to be adjusted for. However, adjusting for baseline providees a more efficient estimator of the direct effect, because it reduces residual variance and eliminates bias from chance baseline imbalance.
...
## Model 3: RCT, Additive Effect, Loss to Follow-Up.
To be finished.
