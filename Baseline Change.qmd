...
3.  An observational study with treatment assignment confounding and loss to follow-up.
...
The Causal DAG for the first and simplest model:
...
# 0: control
# 1: treatment
Y0_time1 <- baseline + 0*additive_effect + rnorm(n, mean = 0, sd = 3)
Y1_time1 <- baseline + 1*additive_effect + rnorm(n, mean = 0, sd = 3)
...
```{r}
table2 <- tbl_summary(dataMod1, by = group) |> add_p() |> bold_labels()
table2
```

Since we have the potential outcomes for all patients we can calculate the Average Treatment Effect of the treatment on time2 as:

```{r}
mean(dataMod1$Y1_time1) - mean(dataMod1$Y0_time1)
```

This is just the mean of the raw individual treatment effects:

```{r}
mean(dataMod1$Y1_time1 - dataMod1$Y0_time1)
```
...
Since the baseline values under treatment and control are identical for each patient, the second and fourth terms in cancel other leaving
