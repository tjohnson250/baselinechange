...
We will also consider two causal estimands based on the two approaches discussed above:

1.  The average treatment effect (ATE) on the outcome at follow-up, expressed as the risk difference between the outcome under treatment vs. control:\
...
There is considerable controversy in the literature over whether and when to use the difference in outcome at follow-up @eq-att-outcome vs the difference in the change score @eq-att-changescore. We will explore some of these issues below.
...
time1_x0 <- baseline + rnorm(n, mean = 0, sd = 3)

# time1 under treatment is on average 10 units higher than baseline
time1_x1 <- baseline + additive_effect + rnorm(n, mean = 0, sd = 3)
...
From the summary, we can see, as expected, that under treatment `time1` and `change1` are close to 10 points higher.

Since we have the potential outcomes for all patients we can calculate the Average Treatment Effect of the treatment on `time1` (@eq-att-outcome) as:
...
We can also compute the difference in *change scores* between the treated and untreated (@eq-changescore):

```{r}
mean(dataMod1$change1_x1) - mean(dataMod1$change1_x0)
```
...
To calculate a p-value and confidence interval, we can do a paired t-test:
...
cat("Mean difference (Treatment - Control):", round(mean_difference, 3), "\n")
cat("95% Confidence Interval:", round(confidence_interval[1], 3), "to", round(confidence_interval[2], 3), "\n")
cat("P-value:", round(p_value, 4), "\n")
```

The paired t-test shows a statistically significant difference between treatment and control conditions (t = 28.61, df = 149, p \< 0.001). Participants showed an average increase of 9.92 units under treatment compared to control (95% CI: 9.23 to 10.60).
...
dataMod2$group <- factor(group, levels = c(1, 0), labels = c("Treatment", "Control"))
...
dataMod2 <- dataMod2 |> select(id, group, baseline, time1, change1)
dataMod2
```
...
A general rule of thumb is that the standardized mean differences between covariates should fall between -0.1 and 0.1. As seen above, despite randomization, the baseline values in our RCT are outside this range. If the baseline values of the treatment and control group are balanced, then an unbiased average treatment effect is just the mean of `time2` for those in the treatment group minus the mean for those in the control group:
...
Recall that the true causal effect is `additive_effect` and the estimate from this data based on potential outcomes is `po_estimate`. The estimate from the RCT is higher than both. Let's do a t-test to test for difference in means for `time1` between the treatment and control groups:

```{r}
t.test(time1 ~ group, data = dataMod2f)
```
...
```{r}
filter(baseline_means, group=="Treatment")$mean - 
...
This estimate removes the bias due to the imbalanced baseline scores. A t-test tests the effect:

```{r}
t.test(change1 ~ group, data = dataMod2, subset = group %in% c("Treatment", "Control"))
```
Here, we see that the means are statistically significantly different. Also, note that tighter confidence interval as compare to the potential outcome estimate.

As we have seen, if the baseline values are unbalanced, the effect estimate of the difference in means of `time2` will be biased. This is why many analysts recommend estimating the mean differences in follow-up scores or change scores using analysis of covariance (ANCOVA) where the outcome is regressed on the treatment and adjusted for the baseline value. 

Let's do this for the change score:

```{r}
res <- lm(change1 ~ group + baseline, data = dataMod2)
summary(res)
```
This does not change the effect estimate in this case, nor does it change the confidence intervals:

```{r}
confint(res, level=0.95)
```
Now lets try ANCOVA on the follow-up score:

```{r}
res <- lm(time1 ~ group + baseline, data = dataMod2)
summary(res)
```
Using ANCOVA on the follow-up scores produces the same estimate as the change scores. Let's look at confidence intervals:

```{r}
confint(res, level=0.95)
```
The confidence intervals are also identical to the change score analysis.
