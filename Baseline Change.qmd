...
1.  The average treatment effect (ATE) on the outcome measure, expressed as the risk difference between the outcome under treatment vs. control:\
    $$\mathrm{E}(Y_1^x) - \mathrm{E}(Y_1^{x'})$$ {#eq-att-outcome}\
    Here, we use the potential outcomes notation, where $Y_1$ is the outcome ($Y_0$ is the baseline score, which is not used in this estimand), $x$ is the treatment, and $x'$ is the control. $Y_1^x$ represents the value that a participant would have if treated. Hence this effect estimate is the difference between the expected outcome if all patients in the study were treated and the expected outcome if all patients were not treated.
...
There is considerable controversy in the literature over whether and when to use the difference in outcomes @eq-att-outcome or the difference in the change score @eq-att-changescore. We will explore some of these issues below.
...
In this model there is no variable for group, because we generate the potential outcomes for all participants under both control and treatment.
...
From the summary, we can see, as expected, that under treatment `time1` and the `change` are close to 10 points higher.
...
When we have all potential outcomes for the particiapants, teh difference in change scores is equal to the difference in outcomes @eq-att-outcome, because the baselines cancel out across the two means below. To see this, we start with the estimand for the change score:
...
When we have the potential outcomes for all patients the baseline values under treatment and control are identical for each patient, hence the second and fourth terms cancel out: $- \mathrm{E}(Y_1^{x'}) + \mathrm{E}(Y_0^{x'})$

```{r}
mean(dataMod1$time1_x1 - dataMod1$baseline) - mean(dataMod1$time1_x0 - dataMod1$baseline)
```
