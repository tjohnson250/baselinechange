---
title: "Claude Change Baseline Censoring"
...
The goal of this notebook is to explain how to estimate the average treatment effect of a dichotomous (binary) treatment on change from a continuous baseline measure [@tennant2021] under three different scenarios:

1.  A Randomized Controlled Trial (RCT) with no loss to follow-up.
2.  An RCT with loss to follow-up.
3.  An observational study with treatment confounders and loss to follow-up.
...
We will use a Doubly Robust (DR) estimator below. For an excellent explanation of DR estimators see [@funk2011]
...
Now let's simulate data for the RCT.
...
```{r}
res <- lm(time2 ~ group + baseline)
summary(res)
```

Here we see that the causal effect of `group` on `time2` when adjusted for baseline is 18.556. Note above the baseline means between treatment and control differ by around .5, which explains the difference between the effect as computed on the difference in the mean of `time2` between groups, vs. the regression results.
...
Interpretation: After adjusting for baseline differences, mean `time2` increased by 13.6 units for those receiving the treatment compared with those who did not receive the treatment. We can see this here:

```{r}
ggplot(data, aes(x = baseline, y = time2, color = group)) + geom_point() + 
...
```{r}
library(dagitty)
library(ggdag) # For nicer visualizations
...
# Convert to tidy format for ggdag
...
adj_sets <- adjustmentSets(dag, exposure = "X", outcome = "Y")
print(adj_sets)

# Highlight adjustment set
ggdag_adjustment_set(tidy_dag, exposure = "X", outcome = "Y") + theme_dag()

# Show all paths
all_paths <- paths(dag, from = "X", to = "Y")
print(all_paths)

# Identify backdoor paths (biasing paths)
backdoor_paths <- all_paths[!all_paths$directed]
print(backdoor_paths)

# Visualize a specific backdoor path
ggdag_path(tidy_dag, from = "X", to = "Y", 
           path = backdoor_paths$paths[[1]]) + theme_dag()
```
...
# Find adjustment sets using dagitty
adj_sets <- adjustmentSets(dag, exposure = "X", outcome = "Y")
cat("Adjustment sets for causal effect of X on Y:\n")
print(adj_sets)

# Using ggdag_adjustment_set to visualize the adjustment set
# First, we need to convert the adjustment set to a character vector
if (length(adj_sets) > 0) {
  first_adj_set <- unlist(adj_sets[[1]])
  
  # Create the adjustment set plot using ggdag_adjustment_set
  p2 <- ggdag_adjustment_set(tidy_dag, 
                           exposure = "X", 
                           outcome = "Y", 
                           adjustment_set = first_adj_set,
                           shadow = TRUE) +
    theme_dag() +
    ggtitle(paste("DAG with Adjustment Set:", paste(first_adj_set, collapse = ", ")))
  print(p2)
}

# Identify biasing paths using pathMarks
biasing_paths <- pathMarks(dag, exposure = "X", outcome = "Y")
cat("\nBiasing paths between X and Y:\n")
print(biasing_paths)

# Visualize all paths between X and Y
p3 <- ggdag_paths(dag_paths(tidy_dag, from = "X", to = "Y")) +
  theme_dag() +
  ggtitle("All Paths Between X and Y")
print(p3)

# Visualize specifically backdoor (biasing) paths
p4 <- ggdag_paths(dag_paths(tidy_dag, from = "X", to = "Y", node_status = "backdoor")) +
  theme_dag() +
  ggtitle("Backdoor (Biasing) Paths")
print(p4)
```
...
# Convert to tidy format for ggdag
tidy_complex_dag <- tidy_dagitty(complex_dag)

# Part 1: Using dagitty for causal analysis
cat("DAGITTY ANALYSIS:\n")
cat("------------------\n")

# Find all adjustment sets
complex_adj_sets <- adjustmentSets(complex_dag, exposure = "X", outcome = "Y")
cat("Adjustment sets for causal effect of X on Y:\n")
print(complex_adj_sets)

# Identify all paths
all_complex_paths <- paths(complex_dag, from = "X", to = "Y")
path_directions <- attr(all_complex_paths, "directed")

# Separate paths into direct (causal) and backdoor (biasing)
direct_paths_idx <- which(path_directions)
backdoor_paths_idx <- which(!path_directions)

cat("\nDirect (causal) paths:\n")
for (i in direct_paths_idx) {
  cat("Path", i, ":", paste(all_complex_paths[[i]], collapse = " -> "), "\n")
}

cat("\nBackdoor (biasing) paths:\n")
for (i in backdoor_paths_idx) {
  cat("Path", i, ":", paste(all_complex_paths[[i]], collapse = " -> "), "\n")
}

# Part 2: Using ggdag for visualization
cat("\nGGDAG VISUALIZATION:\n")
cat("------------------\n")

# Basic DAG visualization
p1 <- ggdag(tidy_complex_dag) + 
  theme_dag() + 
  ggtitle("Basic Complex DAG")
print(p1)

# Show a specific adjustment set
if (length(complex_adj_sets) > 0) {
  for (i in 1:length(complex_adj_sets)) {
    adj_set <- unlist(complex_adj_sets[[i]])
    
    p_adj <- ggdag_adjustment_set(tidy_complex_dag, 
                               exposure = "X", 
                               outcome = "Y", 
                               adjustment_set = adj_set,
                               shadow = TRUE) +
      theme_dag() +
      ggtitle(paste("Adjustment Set", i, ":", paste(adj_set, collapse = ", ")))
    print(p_adj)
  }
}

# Visualize biasing paths
p_backdoor <- ggdag_paths(dag_paths(tidy_complex_dag, 
                                  from = "X", 
                                  to = "Y", 
                                  node_status = "backdoor")) +
  theme_dag() +
  ggtitle("Backdoor (Biasing) Paths")
print(p_backdoor)

# Visualize all paths
p_all_paths <- ggdag_paths(dag_paths(tidy_complex_dag, 
                                   from = "X", 
                                   to = "Y")) +
  theme_dag() +
  ggtitle("All Paths Between X and Y")
...
```{r}
# Example: Focusing on a confounding path and its closure
...
# Identify the backdoor path: X <- Z -> Y
# Visualize this specific path (can be manual for clarity in teaching)
# For example, by drawing arrows with specific colors or styles if ggdag_paths is not direct enough.

# Show the adjustment set
ggdag_adjustment_set(confounder_dag, shadow = TRUE) +
  labs(title = "Adjusting for Z closes the backdoor path X <- Z -> Y") +
  theme_dag()

# Illustrating a path opened by adjusting a collider
m_bias_dag <- dagify(
  Y ~ M2,
  X ~ M1,
  M1 ~ U1 + C,
  ...
  exposure = "X",
  outcome = "Y",
  latent = c("U1", "U2"), # Unobserved variables
  # coords = list(...) # Add coordinates for better layout
)

# Plot with C unadjusted - X and Y are d-separated
ggdag_dseparated(m_bias_dag, from = "X", to = "Y", controlling_for = NULL) +
  theme_dag() +
  labs(title = "M-bias: X and Y are d-separated (no adjustment)")

# Plot with C adjusted - path X <- M1 <- C -> M2 -> Y opens!
# (Actually, X <- M1 <- U1 -- C -- U2 -> M2 -> Y if U1, U2 are common causes with C)
# Let's simplify to illustrate opening via C:
# X <- A -> C <- B -> Y. If we control C, a path between X and Y might open via A and B.

collider_path_dag <- dagify(
  X ~ A,
  Y ~ B,
  C ~ A + B, # C is a collider
  exposure = "X",
  outcome = "Y"
  # coords = list(...)
)

# Nothing adjusted: X _||_ Y
ggdag_dseparated(collider_path_dag, from = "X", to = "Y", controlling_for = NULL) +
  theme_dag() + labs(title = "Collider: X and Y are d-separated")

# Adjust for C: an inducing path X ~ A -> C <- B ~ Y is created
ggdag_dseparated(collider_path_dag, from = "X", to = "Y", controlling_for = "C") +
  theme_dag() +
  labs(title = "Collider: Adjusting for C opens a biasing path between X and Y")
```
