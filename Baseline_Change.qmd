...
To create the weights, we use logistic regression to fit a censoring model using all covariates that directly affect censoring. For our model, we need only predict remaining uncensored based on the covariate `W` since, as you can see from the causal DAG above, W is the only parent of `censored`.
...
censoring_model <- glm(uncensored ~ group  + L + W, data = dataMod3, family = binomial)
...
dataMod3$prob_uncensored <- predict(censoring_model, type = "response")
```
...
treated_mean <- weighted.mean(uncensored_data$time1[uncensored_data$group == "Treatment"],
                              uncensored_data$ipcw_weight[uncensored_data$group == "Treatment"])

# Weighted mean for control group (group = 0)
control_mean <- weighted.mean(uncensored_data$time1[uncensored_data$group == "Control"], 
                             uncensored_data$ipcw_weight[uncensored_data$group == "Control"])
