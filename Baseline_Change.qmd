...
Recall that the true causal effect is `{r} additive_effect` and the estimate from this data based on potential outcomes is `{r} po_estimate`. The estimate from the RCT is lower than both. Let's use regression to estimate the difference in means for `time1` between the treatment and control groups and give us a p-value and confidence interval:
...
The square around `censored` and the color shows us that `censored` is now adjusted. As a result, the dashed line indicates that association can flow between `group` and `L` and then on to `time`.
