...
#| echo: false
library(cobalt)
unadjcov <- bal.tab(group ~ baseline + L, data = dataMod2, thresholds = c(m = .1))
...
`group -> time1` is a causal path that is open, meaning that association can flow through it. `group -> censored <- L -> time1` is a backdoor path that is closed, because association is blocked because `censored` is a collider on this path. A collider is a node with both arrows pointing toward it. Colliders block the flow of association, as long as we don't condition or adjust for the collider. However, if we analyze only patients who remained in the study (were not censored) then we are selecting only uncensored patients, which means that we are setting $censored = 1$, hence we are adjusting `censored`. This opens the flow of association along the backdoor path, biasing our effect estimates.
...
According to Pearl's causal inference framework [@pearl2010], adjusting for `L` will block the non-causal path. One way to do this is by adding `L` as a covariate to the regression:
