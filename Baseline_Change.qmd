...
W: A (binary) side-effect resulting from a combination of `L` and the treatment, `group`. W is more likely when L=1 and when treated `group = treated`. We will use `W` later to model to loss to follow-up.
