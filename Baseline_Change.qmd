...
Sample code for using mathematical symbols as labels in DAGs. I need this to convert to the unified labeling system.

```{r}
library(ggdag)
library(ggplot2)

# Create DAG with LaTeX-style math labels
dag <- dagify(
  Y ~ X + Z,
  X ~ Z,
  labels = c(
...
# Modify labels to use mathematical expressions
dag$data$label_math <- case_when(
  dag$data$name == "Y" ~ "Y[1]",
  dag$data$name == "X" ~ "Y[0]", 
  dag$data$name == "Z" ~ "Delta*Y",
  TRUE ~ dag$data$name
)

# Plot with math expressions
ggdag(dag) +
  geom_dag_node(aes(color = name)) +
  geom_dag_text(aes(label = label_math), parse = TRUE, size = 3) +
  geom_dag_edges() +
  theme_dag()
...
# Add math labels and status
dag$data$math_label <- case_when(
  dag$data$name == "Y" ~ "Delta*Y",
  dag$data$name == "X" ~ "X",
  dag$data$name == "Z" ~ "Z", 
  dag$data$name == "W" ~ "W",
  TRUE ~ dag$data$name
...
# Define status for each node
dag$data$status <- case_when(
  dag$data$name == "Y" ~ "Outcome",
  dag$data$name == "X" ~ "Treatment", 
  dag$data$name == "Z" ~ "Adjusted",
  dag$data$name == "W" ~ "Unadjusted",
  TRUE ~ "Other"
)

# Plot with custom legend
ggdag(dag) +
  geom_dag_node(aes(color = status), size = 15) +
  geom_dag_text(aes(label = math_label), parse = TRUE, size = 4, color = "white") +
  geom_dag_edges() +
  scale_color_manual(
...
coords <- list(
  x = c(Y = 4, X = 2, Z1 = 1, Z2 = 3, U1 = 0, U2 = 2, C = 1.5),
  y = c(Y = 1, X = 1, Z1 = 2, Z2 = 2, U1 = 3, U2 = 3, C = 0)
)
...
dag$data$math_label <- case_when(
  dag$data$name == "Y" ~ "Delta*Y",
  dag$data$name == "X" ~ "X",
  dag$data$name == "Z1" ~ "Z[1]",
  dag$data$name == "Z2" ~ "Z[2]",
  dag$data$name == "U1" ~ "U[1]",
  dag$data$name == "U2" ~ "U[2]", 
  dag$data$name == "C" ~ "C"
)
...
dag$data$status <- case_when(
  dag$data$name == "Y" ~ "Outcome",
  dag$data$name == "X" ~ "Treatment",
  dag$data$name %in% c("Z1", "Z2", "C") ~ "Adjusted",
  dag$data$name %in% c("U1", "U2") ~ "Unobserved"
)
...
In this notebook we use the standard notation from the potential outcomes approach to refer the variables. This matches the notation used in the change score analysis in [@tennant2021].
...
4.  The conditional treatment effect on the follow-up outcome, conditioned on the observed baseline value $Y_0$:\
    $$
...
5.  The conditional treatment effect on the change score, conditioned on the observed baseline value $Y_0$:\
    $$
...
dagMod1 <- dagify(
  ...
  outcome = "time1",
  coords = data.frame(
    name = c("baseline", "L", "W", "group", "time1", "change1"),
    x = c(0, 0, 2, 4, 3, 1.5)*.5,
    y = c(1, 2, 2.5, 2, 1, 0)*.5
  )
)
